...
export default {
	...
	methods: {
		...
		itemIsDisabled (index) { // 某个列表项是否已经被禁用了
			return (this.innerList[index] && this.innerList[index].disabled) || false;
		},
...
@mouse-move-color: transparent; //在列表项上按下时的列表项的背景色
...
.show-box {
	...
	.list-container {
		...
		.popper__arrow {
			...
			filter: drop-shadow(0 1.38upx 8.33upx rgba(0, 0, 0, 0.03));
			left: 30%;
			margin-right: 2.08upx;
			border-top-width: 0;
		...
		.list {
			...
			border: 0.69upx solid #1b92ef;
			width: 100%;
			max-height: 10em;
			background-color: #fff;
			box-shadow: 0 1.38upx 8.33upx 0 rgba(0, 0, 0, 0.1);
			padding: 3.47upx 0;
