  ...
  <div id="multi-selector-picker-view" @click="isShowSelector = !isShowSelector">
    ...
    <div class="picker-view-box" :class="{ 'picker-in-box': !isInFullPage }" v-show="isShowSelector">
      <div class="picker-view-warrper keyboard-img">
        <picker-view v-for="(items, index) in range" :key="index" :value="[value[index]]" :data-index="index" @change="change" class="picker-view" indicator-style="height: 50px;" :mask-style="maskStyle" :style="pickerViewStyle">
          <picker-view-column>
...
export default {
  data () {
    return {
      ...
      // maskStyle: `background: linear-gradient(180deg,hsla(0,0%,100%,.5),hsla(0,0%,100%,.3)),linear-gradient(0deg,hsla(0,0%,100%,.5),hsla(0,0%,100%,.3));
