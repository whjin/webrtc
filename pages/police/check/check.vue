...
export default {
	...
	methods: {
		...
		// 上传图片
		uploadImg () {
			currentPage().handleShowToast("提交保存登记中...", "center");
		...
		save () {
			if (!this.saveCheckParams.startTime) {
				return currentPage().handleShowToast("请选择开始检查时间！", "center");
			}
			if (!this.saveCheckParams.inspectionType) {
				return currentPage().handleShowToast("请选择检查类型！", "center");
			}
			if (!this.saveCheckParams.participantsNum) {
				return currentPage().handleShowToast("参加人数请输入数字！", "center");
			}
			if (!this.saveCheckParams.checkContent.length) {
				return currentPage().handleShowToast("请添加检查内容！", "center");
			}
			this.uploadImg();
		},
		// 保存登记
		async saveCheck () {
			...
			if (res.state.code == 200) {
				currentPage().handleShowToast(res.data, "center");
