  ...
  methods: {
    ...
    submit() {
      if (this.currentPage == 1 && !this.takeOutType) {
        return currentPage().handleShowToast("请选择外出原因！", "center");
      }
      this.showComfirmDialog = false;
      // 人脸验证开关
    ...
    fingerRecognitionSuccess(res) {
      ...
      if ([mKey, mKey2].includes(res.mKey)) {
        ...
        currentPage().voiceBroadcast("验证不通过，请确认在押人员信息");
      }
    },
    // 验证成功
    verifySuccess() {
      currentPage().voiceBroadcast("验证通过");
      setTimeout(() => {
    ...
    async handleTakePrisoner() {
      ...
      if (res.state.code == 200) {
        ...
        if (this.currentPage == 1) {
          currentPage().handleShowToast("操作失败！", "center");
        } else {
    ...
    changePrisonerStatus() {
      ...
      currentPage().handleShowToast("操作成功！", "center");
    },
    // 关闭弹框
    closeComfirmDialog() {
      this.takeOutType = "";
