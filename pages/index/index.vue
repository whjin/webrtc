...
export default {
	...
	methods: {
		...
		async getOnlineAlermInfo () {
			const { areaId: id, controlCode } = uni.getStorageSync("controlInfo");
			let res = await Api.apiCall("get", Api.index.getOnlineAlermInfo, { id, controlCode });
		...
		async getAllAlerm () {
			const { areaId: id } = uni.getStorageSync("controlInfo");
			let res = await Api.apiCall('get', Api.index.getAllAlarm, { id });
			if (res.state.code == '200') {
				// 获取横坐标日期
				let dateList = [];
				res.data.map(item => {
		...
		async getTerminalAlarm () {
			const { areaId: id } = uni.getStorageSync("controlInfo");
			let res = await Api.apiCall('get', Api.index.getTerminalAlarm, { id });
			if (res.state.code == '200') {
				// 获取分机横坐标监室号
				let prisonList = [];
				res.data.map(item => {
