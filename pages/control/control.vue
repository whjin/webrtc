<template>
  <div class="control-container">
    <div class="control-wrapper">
      ...
      <div class="control-center">
        <div class="control-center-head">
          <div class="center-head-box">
            <div class="head-tabs" v-for="(item) in controlList" :key="item.index">
        ...
        <div class="control-center-table">
              ...
              <div>
                <div class="control-table-box">
                  <div class="table-head">
                    ...
                    <div class="control-head-item" v-for="(item, index) in columns" :key="index">
                      {{ item.title }}
                    </div>
                  </div>
                  <scroll-view scroll-y="true" class="control-table-scroll">
                    <div class="control-table-main" v-for="(item, index) in audioTableList" :key="index">
                      <div class="table-content">
                        ...
                        <div class="control-table-item" style="flex: 1">
                          {{ item.site }}
                        </div>
                        <div class="control-table-item" style="flex: 1"
      ...
      <div class="control-right" @click="changeStatus">
        <div class="control-right-title">人员信息</div>
    ...
    <div class="neil-modal-container">
      ...
      <neil-modal :show="showAddGroup" @close="closeModal('AddGroup')">
        <div class="control-modal-container" style="width: 480upx; height: 320upx">
          <div class="neil-modal-label">
            确认{{
              currentPage > 4
              ? isRead
...
import { unique, uniqueArr, dateFormat, currentPage } from "@/common/utils/util.js";
import controlList from "@/static/mock/control/controlList.json";
import controlColumns from "@/static/mock/control/controlColumns.json";
import roomColumns from "@/static/mock/control/roomColumns.json";
import doorColumns from "@/static/mock/control/doorColumns.json";
import temColumns from "@/static/mock/control/temColumns.json";
import otherColumns from "@/static/mock/control/otherColumns.json";
import controlTableList from "@/static/mock/control/controlTableList.json";
...
export default {
  ...
  computed: {
    ...
    columns() {
      let columns = this.controlColumns;
  ...
  methods: {
    ...
    searchTerminalRoom() {
      if (!this.searchTerminal) {
        currentPage().handleShowToast("请输入搜索内容", "center");
        return;
      }
    ...
    checkAudioAll() {
      if (this.audioState) {
        currentPage().handleShowToast("请先停止分机播放", "center");
        return;
      }
      if (!this.audioTableList.length) {
        return;
    ...
    handleOpenAudio() {
      ...
      if (this.audioState) {
        ...
        currentPage().sendWebsocket(JSON.stringify(controlObj));
        let params = {
          controlId: uni.getStorageSync("controlInfo").id,
          type: "300",
    ...
    async setDynamicInfo(params) {
      ...
      if (res.state.code == 200) {
        ...
        currentPage().handleShowToast("请求错误", "center");
      }
    },
    // 分机连接状态
    getConnectInfo(data) {
      this.roomTableList.map((item, index) => {
        data.map((res) => {
          if (res.name == item.name) {
            item.status = res.status == "0" ? "连接成功" : "连接失败";
    ...
    handleAddGroup(status) {
      if (!this.selectSongNum) {
        currentPage().handleShowToast("请先选择监室", "center");
        return;
      }
      this.isRead = status;
    ...
    addGroupConfirm() {
      if (this.currentPage < 5) {
        ...
        currentPage().handleShowToast("上传成功！", "center");
      }
    ...
    async controlHandler(uri) {
      let res = await Api.apiEsCall("get", Api.test.controlOut + uri, null);
      if (res.state.code == 200) {
        currentPage().handleShowToast("指令下发成功！");
      }
    },
    addGroup() {
    ...
    closeModifyGroup() {
      ...
      currentPage().handleShowToast("一键撤防成功！", "center");
    },
    // 获取修改分组名称
    modifyGroupChange(e) {
    ...
    modifyGroupConfirm() {
      ...
      //   currentPage().handleShowToast("请输入分组名称", "center");
      //   return;
      // }
      ...
      this.selectSongNum = 0;
      this.isAudioAll = false;
      currentPage().handleShowToast("一键布防成功！", "center");
