...
import { dateFormat, timeFormat, currentPage } from "@/common/utils/util.js";
...
export default {
  ...
  methods: {
    ...
    async setDynamicInfo(state) {
      ...
      if (res.state.code == 200) {
        currentPage().handleShowToast("保存视频点播动态成功！");
      } else {
        currentPage().handleShowToast("请求错误！");
      }
    },
    // 选择视频
