    ...
    <div class="question-wrapper">
      ...
      <main class="question-content">
        ...
        <div class="content-title">
          管教测评
          <section>
            共<span>{{ subjectList.length }}</span>项，当前第<span>{{ currentIndex + 1 }}</span>项
          </section>
        </div>
...
import { countDown, currentPage } from "@/common/utils/util.js";
...
export default {
  ...
  created() {
    this.getSubjectList(this.talkId);
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    async getSubjectList(id) {
      ...
      if (res.state.code == "200") {
        ...
        this.countTime = countDown(res.data.talkTime);
        this.countDownTimer(res.data.talkTime);
      }
    },
    // 剩余时间倒计时
    countDownTimer(sec) {
    ...
    prevSubject() {
      ...
      if (this.currentIndex < 0) {
        this.currentIndex = 0;
        currentPage().handleShowToast("当前已是第一题", "center");
        return;
      }
    },
    // 下一题
    nextSubject() {
      ...
      if (this.currentIndex > lastIndex) {
        ...
        currentPage().handleShowToast("当前已是最后一题", "center");
        return;
      }
    },
    // 提交
