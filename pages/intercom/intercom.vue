  ...
  methods: {
    ...
    handleTabChange(page) {
      if (this.openIntercom) {
        currentPage().handleShowToast("请先挂断视频", "center", 5000);
        return;
      }
      this.page = page;
      this.$nextTick(() => {
    ...
    controlChatConfirm() {
      if (!this.isRepeatState) {
        ...
        currentPage().voiceBroadcast("正在发起视频通话");
        const { controlCode } = uni.getStorageSync("controlInfo");
    ...
    // 挂断视频通话处理
    stopIntercomHandler() {
      // currentPage().voiceBroadcast("对讲已挂断");
      this.clearCallTimer();
    ...
    async saveRadioPlayStatus(controlCode, status) {
      ...
      if (res.state.code == 200) {
        currentPage().handleShowToast("保存状态成功", "bottom");
      }
    },
    // 关闭对讲语音播报定时器
    clearCallTimer() {
    ...
    startCallTimer(content) {
      currentPage().voiceBroadcast(content);
    ...
    handleAnswerCall() {
      if (this.openIntercom) {
        if (this.isPolling) {
          ...
          currentPage().handleShowToast("请先挂断当前对讲", "center");
        }
      } else {
        if (!currentPage().isAlarmRepeat && !this.isHangupRepeat && !this.isRepeatState) {
          this.answerCallHandler();
        }
      }
    },
    ...
    intercomVolume() {
      ...
      // 分机音量调节
      if (Object.keys(currentPage().terminalInfo).length) {
        const { maindevno, devno } = currentPage().terminalInfo;
        currentPage().sendWebsocket(
