    ...
    <div class="shopping-wrapper">
      <div class="shopping-header">
        ...
        <div class="shopping-header-right">
          <div class="consume" @click="openShoppingRecord">
            <common-icons iconType="iconconsume" size="48" color="#fff" />
          </div>
          <div class="cart" @click="openShoppingCart">
                ...
                <div class="purchase">
                  ...
                  <div class="count">
                    <div class="minus-btn" :class="{ 'minus-disabled': item.disabled }" @click="handleMinusGoods(item)">
                      ...
                      <div class="plus-btn" @click="handlePlusGoods(item)">
      ...
      <neil-modal :show="showShoppingCart" @close="closeModal('ShoppingCart')">
        <div class="cart-modal-box">
          <div class="modal-header">
            ...
            <div class="modal-close" @click="closeModal('ShoppingCart')">
                    ...
                    <div class="table-content" v-for="(item, index) in cartGoodsList" :key="index">
                      ...
                      <div class="cart-table-item" style="flex: 2">
                        <div class="count">
                          ...
                          <div class="num">{{ item.selectNum }}</div>
                          <div class="plus-btn" @click="handlePlusCart(item, index)">
                            <common-icons iconType="iconplus" size="18" color="#fff" />
                          </div>
                        </div>
                      </div>
        ...
        <neil-modal :show="showShoppingRecord" @close="closeModal('ShoppingRecord')">
          <div class="record-modal-box">
            <div class="modal-header">
              ...
              <div class="modal-close" @click="closeModal('ShoppingRecord')">
                <image src="/static/images/common/close.png"></image>
              </div>
            </div>
        ...
        <neil-modal :show="showOrderInit" @close="closeModal('OrderInit')">
          <div class="order-modal-box">
            <view class="modal-header">
              ...
              <div class="modal-close" @click="closeModal('OrderInit')">
                <image src="/static/images/common/close.png"></image>
              </div>
            </view>
        ...
        <neil-modal :show="showCrossMonthModal">
          <div class="common-modal-container">
            ...
            <div class="common-modal-button">
              <div class="btn-confirm" @click="closeModal('CrossMonthModal')">
                确定
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    async getPrisonInfo(code) {
      ...
      if (res.state.code == 200) {
        this.accountBalance = res.data.money
          ? formatFloat(res.data.money, 2)
          : 0;
        this.prisonerInfo = res.data;
        this.regConfig.regName = this.prisonerInfo.name;
        this.regConfig.rybh = this.prisonerInfo.rybh;
        let params = {
    ...
    handleClearCart() {
      if (!this.cartGoodsList.length) {
        currentPage().handleShowToast("购物车为空", "center");
        return;
      }
      // 初始化商品信息
      this.initGoodsInfo();
    ...
    async getOrderHistoryInfo() {
      ...
      if (selectDateList.includes("")) {
        currentPage().handleShowToast("请选择订单日期", "center");
        return;
      }
      let params = {
        data: {
    ...
    async getOrderDetail(detailNo) {
      ...
      if (res.state.code == 200) {
        ...
        currentPage().handleShowToast("获取消费详情失败", "center");
      }
    },
    // 关闭消费记录详情
    ...
    async confirmGoods(goods) {
      ...
      if (res.state.code == 200) {
        ...
        this.$forceUpdate();
        currentPage().handleShowToast("确认成功", "center");
      } else {
        currentPage().handleShowToast("确认失败", "center");
      }
    },
    ...
    async shortcutConfirm() {
      ...
      if (res.state.code == 200) {
        ...
        this.$forceUpdate();
        currentPage().handleShowToast("一键确认成功", "center");
      } else {
        currentPage().handleShowToast("一键确认失败", "center");
      }
    },
    // 设置消费记录分页参数
