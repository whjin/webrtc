<template>
	<div class="conversation-container">
		...
		<recognition-dialogs ref="recognitionDialogs" useFor="conversation" :regConfig="regConfig" :isShow="showRecognitionDialogs" @fingerRecognitionSuccess="fingerRecognitionSuccess" @faceRecognitionSuccess="faceRecognitionSuccess" @close="closeRecognitionDialogs"></recognition-dialogs>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		async getSelfTalkInfo () {
			...
			let res = await Api.apiCall("post", Api.index.getTalkInfo + `?id=${this.taskId}&roomNo=${roomNo}`, null);
		...
		recognitionHandle () {
			...
			this.$nextTick(() => {
				this.$refs.recognitionDialogs && this.$refs.recognitionDialogs.startRecognition(19);
			});
		},
		// 指纹认证成功回调
		...
		async getLoginPersonInfo (mKey) {
			...
			let res = await Api.apiCall("get", Api.index.getOdsPrisonerInfo, params, true);
			if (res.state.code == 200) {
