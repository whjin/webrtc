<template>
  <div class="rotation-container">
    <div class="rotation-wrapper">
      <div class="rotation-content">
        <div class="rotation-title">
          <text>值班签到</text>
          <div class="button" v-if="currentPage == 1 && dutyPersonList.length" @click="startDutyCall">
            开始签到
          </div>
        </div>
        <div class="rotation-box">
          ...
          <div v-if="currentPage == 3" class="rotation-record-container">
            ...
            <div class="record-table-box">
              ...
              <scroll-view scroll-y="true" class="record-table-scroll" @scrolltolower="recorListScrolltolower">
    ...
    <recognition-dialogs ref="recognitionDialogs" useFor="rotation" :regConfig="regConfig"
  ...
  computed: {
    ...
    onDutyPerson() {
      return this.dutyList.length
        ? this.dutyList.map((item) => item.name).toString()
        : "";
    },
  },
  mounted() {
  ...
  methods: {
    ...
    startDutyCall() {
      if (this.dutyList.length) {
        ...
        console.log("当前没有值班任务");
        currentPage().handleShowToast("当前没有值班任务");
      }
    },
    // 指纹认证成功回调
    ...
    handleRegSubmit() {
      ...
      if (!hadAuth) {
        currentPage().handleShowToast("没有提交权限");
        return;
      }
      if (!this.dutyStatus) {
        currentPage().handleShowToast("请选择值班情况");
        return;
      }
    ...
    async updateSituation() {
      ...
      if (res.state.code == 200) {
        currentPage().handleShowToast("提交成功");
      }
    },
    // 取消值班登记
    handleRegCancel() {
    ...
    async signRecordSearch() {
      ...
      if (res.state.code == 200) {
        if (this.dutyRecordPageParam.pageIndex == 1 && !res.data.length) {
          currentPage().handleShowToast("暂无更多数据", "center");
        }
