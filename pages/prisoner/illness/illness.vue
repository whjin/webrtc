...
import { dateFormat, uniqueArr, currentPage } from "@/common/utils/util.js";
...
export default {
  ...
  mounted() {
    ...
    currentPage().voiceBroadcast("请将手掌移至屏幕正上方测温仪进行测温");
    // 预约就医类型
    this.getIllnessTypeInfo();
  },
  destroyed() {
    // 停止测温
    currentPage().closeThermometryModule();
  },
  methods: {
    // 菜单切换
    switchPage(index) {
    ...
    async getIllnessTypeInfo() {
      currentPage().openThermometryModule();
    ...
    reserveSubmit() {
      if (
        ...
        currentPage().handleShowToast("请选择就医类型", "center");
        return;
      }
      if (this.foreheadTemperature < this.lowTemperature) {
        currentPage().handleShowToast("请先进行测温", "center");
        return;
      }
      let params = {
    ...
    async handleIllnessUrge(item) {
      if (!this.isRepeatState) {
        ...
        if (res.state.code == 200) {
          currentPage().handleShowToast("催办成功", "center");
        }
      }
    },
    // 选择症状开始时间
    ...
    handleRecordQuery() {
      ...
      if (selectDateList.includes("")) {
        currentPage().handleShowToast("请选择上报时间", "center");
        return;
      }
      this.pageIndex = 1;
      this.recordList = [];
